...
> **定位说明**：本 skill 适用于只需 PPT→合成图、不需要上传飞书的场景。如果需要全链路（含封面放置、上传发布），请使用 **zhifa-pipeline**。本 skill 调用 `pipeline.py run`，不支持 `--cover-source` 封面放置参数。
...
可选参数（不说就用默认值）：
- **模板**：默认全部可用模板（不传 `--templates` 时脚本自动获取）；用户说「只用模板1和2」时传 `--templates 1 2`
